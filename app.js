...
const state = {
  ...
  autoBuild: { running:false, maxRepairLoops:3, lastStatus:null },
  lastPackage: null,
  layoutCache: { key: null, pos: null },
  projectAnalysis: null,
  aiTeam: null,
  aiKeys: { openai:'', anthropic:'' },
  activeAiRole: 'builder',
};
...
function defaultAiTeam(){
  return {
    mode:'dual',
    roles:{
      architect:{label:'Architect', provider:'anthropic', model:'claude-sonnet-4-6', enabled:true, job:'Blueprint, planning, architecture, risk review'},
      builder:{label:'Builder', provider:'openai', model:'gpt-5.5', enabled:true, job:'Generate code, refactor sections, apply repairs'},
      tester:{label:'Tester', provider:'openai', model:'gpt-5.5', enabled:true, job:'Write tests, inspect failures, propose fixes'},
      security:{label:'Security', provider:'anthropic', model:'claude-sonnet-4-6', enabled:true, job:'Auth, data, API key, webhook, injection and permission review'},
      reviewer:{label:'Reviewer', provider:'anthropic', model:'claude-sonnet-4-6', enabled:true, job:'Second opinion, blueprint diff review, quality check'}
    }
  };
}
function ensureAiTeam(){ if(!state.aiTeam) state.aiTeam=defaultAiTeam(); return state.aiTeam; }
function roleForIntentText(text=''){
  const t=String(text).toLowerCase();
  if(/security|auth|permission|stripe|webhook|secret|risk/.test(t)) return 'security';
  if(/test|jest|pytest|failure|error|run/.test(t)) return 'tester';
  if(/review|diff|explain|why|architecture|plan/.test(t)) return 'reviewer';
  if(/blueprint|design|structure|wiring/.test(t)) return 'architect';
  return 'builder';
}
function roleConfig(role){
  const team=ensureAiTeam();
  if(team.mode==='openai_only') return {label:'OpenAI', provider:'openai', model:team.roles.builder.model||'gpt-5.5', job:'All work'};
  if(team.mode==='claude_only') return {label:'Claude', provider:'anthropic', model:team.roles.architect.model||'claude-sonnet-4-6', job:'All work'};
  return team.roles[role] || team.roles.builder;
}
function setActiveAiRole(role){ state.activeAiRole=role||'builder'; const cfg=roleConfig(state.activeAiRole); const ps=document.getElementById('providerSelect'); const mi=document.getElementById('modelInput'); const ki=document.getElementById('apiKeyInput'); if(ps) ps.value=cfg.provider; if(mi) mi.value=cfg.model; if(ki && state.aiKeys[cfg.provider]) ki.value=state.aiKeys[cfg.provider]; updateProviderUi(); }
function saveAiTeamFromUi(){
  const team=ensureAiTeam();
  const mode=document.getElementById('aiTeamMode'); if(mode) team.mode=mode.value;
  state.aiKeys.openai=document.getElementById('openaiTeamKey')?.value.trim()||state.aiKeys.openai||'';
  state.aiKeys.anthropic=document.getElementById('anthropicTeamKey')?.value.trim()||state.aiKeys.anthropic||'';
  Object.keys(team.roles).forEach(id=>{
    const en=document.getElementById(`role_${id}_enabled`); const pr=document.getElementById(`role_${id}_provider`); const mo=document.getElementById(`role_${id}_model`); const jo=document.getElementById(`role_${id}_job`);
    if(en) team.roles[id].enabled=en.checked; if(pr) team.roles[id].provider=pr.value; if(mo) team.roles[id].model=mo.value.trim(); if(jo) team.roles[id].job=jo.value.trim();
  });
  autosave(); renderAiTeam(); toast('AI team saved');
}
function useRecommendedAiTeam(){
  state.aiTeam=defaultAiTeam(); state.aiTeam.mode='dual';
  renderAiTeam(); saveAiTeamFromUi(); setActiveAiRole('builder'); toast('Recommended dual mode enabled');
}
function renderAiTeam(){
  const team=ensureAiTeam();
  const mode=document.getElementById('aiTeamMode'); if(mode) mode.value=team.mode;
  const ok=document.getElementById('openaiTeamKey'); if(ok && !ok.value) ok.value=state.aiKeys.openai||'';
  const ak=document.getElementById('anthropicTeamKey'); if(ak && !ak.value) ak.value=state.aiKeys.anthropic||'';
  const list=document.getElementById('aiTeamRoleList');
  if(list){
    list.innerHTML=Object.entries(team.roles).map(([id,r])=>`<div class="aiRoleRow">
      <div class="roleName">${escapeHtml(r.label)}<small>${escapeHtml(id)}</small></div>
      <label class="inlineCheck"><input id="role_${id}_enabled" type="checkbox" ${r.enabled?'checked':''}/> Enabled</label>
      <select id="role_${id}_provider"><option value="openai" ${r.provider==='openai'?'selected':''}>OpenAI</option><option value="anthropic" ${r.provider==='anthropic'?'selected':''}>Claude</option></select>
      <input id="role_${id}_model" value="${escXml(r.model)}" />
      <input id="role_${id}_job" value="${escXml(r.job)}" />
    </div>`).join('');
  }
  const summary=document.getElementById('aiTeamRoutingSummary');
  if(summary){
    const modeText={openai_only:'OpenAI handles every role.', claude_only:'Claude handles every role.', dual:'OpenAI builds/tests; Claude plans/reviews/security.', specialist:'Each role uses its assigned provider/model.'}[team.mode]||team.mode;
    summary.innerHTML=`<div class="aiRouteCard"><div><b>Mode</b><span>${escapeHtml(modeText)}</span></div><span class="badge live">${escapeHtml(team.mode)}</span></div>`+
      Object.entries(team.roles).map(([id,r])=>`<div class="aiRouteCard"><div><b>${escapeHtml(r.label)}</b><span>${escapeHtml(r.job)}</span></div><div><span class="badge">${r.provider==='openai'?'OpenAI':'Claude'}</span><span>${escapeHtml(r.model)}</span></div></div>`).join('');
  }
}
...
function getProvider() {
  const cfg = roleConfig(state.activeAiRole || 'builder');
  return cfg?.provider || document.getElementById('providerSelect')?.value || 'anthropic';
}
...
function getApiKey() {
  ...
  const visible = document.getElementById('apiKeyInput')?.value.trim() || '';
  return visible || state.aiKeys?.[provider] || '';
}

function getModelId() {
  const cfg = roleConfig(state.activeAiRole || 'builder');
  return (cfg?.model || document.getElementById('modelInput')?.value || providerDefaults[getProvider()]?.model || '').trim();
}
...
function buildSystemPrompt() {
  ...
  const roleCfg = roleConfig(state.activeAiRole || 'builder');
  const parts = [
    'You are WireStack AI — an expert software architect and developer embedded in the WireStack Studio.',
    `Active AI role: ${roleCfg.label || state.activeAiRole}`,
    `Provider role job: ${roleCfg.job || 'General build work'}.`,
    'Your job is to analyze blueprints, generate real working code, write tests, and provide repair suggestions.',
    'Rules:',
    '- Return code in fenced code blocks with language tags.',
...
function aiSend() {
  ...
  setActiveAiRole(roleForIntentText(userMsg));
  ...
  sendToAiStreaming(
    ...
    (fullText) => {
      ...
      if (fullText) {
        ...
        if (blocks.length > 0) { applyBtn.style.display = 'inline-flex'; document.getElementById('runnerApplyBtn').style.display='inline-flex'; document.getElementById('runnerTestBtn').style.display='inline-flex'; const pbtn=document.getElementById('runnerPackageBtn'); if(pbtn) pbtn.style.display='inline-flex'; }
        renderAiHistory();
        autosave();
...
function extractAllFencedBlocks(text) {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    const fileLine = before.match(/(?:^|\n)\s*(?:#{2,6}\s*)?(?:File|Path)\s*:\s*`?([^`\n]+?)`?\s*$/i);
    const firstLine = code.split('\n')[0] || '';
    const commentPath = firstLine.match(/(?:\/\/|#|--|\/\*)\s*(?:File\s*:\s*)?([\w./\\-]+\.[a-z0-9]+)/i);
    const moduleMatch = firstLine.match(/(?:\/\/|#|--|\/\*)\s*([a-z0-9_.-]+(?:\.[a-z]+)?)/i);
    const filePath = fileLine ? fileLine[1].trim() : (commentPath ? commentPath[1].trim() : '');
    const label = filePath || (moduleMatch ? moduleMatch[1] : `block-${blocks.length + 1}`);
    blocks.push({ lang, code, label, path: filePath, index: blocks.length });
  }
  return blocks;
}

function inferRunnerPath(block, index){
  if(block.path) return block.path;
  const label=(block.label || `generated-${index+1}`).trim();
  if(/[\\/]/.test(label) && /\.[a-z0-9]+$/i.test(label)) return label.replace(/\\/g,'/');
  const lang=(block.lang||'').toLowerCase();
  const ext=lang.includes('sql')?'sql':lang.includes('python')||lang==='py'?'py':lang.includes('html')?'html':lang.includes('css')?'css':lang.includes('json')?'json':lang.includes('bash')||lang==='sh'?'sh':'js';
  return `generated/${label.replace(/[^a-z0-9._-]+/gi,'-').replace(/^-+|-+$/g,'') || 'block-'+(index+1)}.${ext}`;
}
  ...
  el.innerHTML = blocks.map((b, i) => `
    <div class="codeBlockCard">
      <div class="codeBlockHeader">
        ...
        <span class="codeBlockLabel">${escapeHtml(b.path || b.label)}</span>
...
async function runnerRequest(path, body){
  const url=(state.runnerUrl || 'http://localhost:8787') + path;
  const opts=body?{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}:{method:'GET'};
  const res=await fetch(url, opts);
  const data=await res.json().catch(()=>({error:'Invalid runner response'}));
  if(!res.ok) throw new Error(data.error || ('Runner HTTP '+res.status));
  return data;
}

function setRunnerReport(report){
  state.runnerReport=report;
  const el=document.getElementById('runnerReport');
  if(el) el.textContent=typeof report==='string'?report:JSON.stringify(report,null,2);
  autosave();
}

async function runnerPing(){
  try{
    const data=await runnerRequest('/api/health');
    setRunnerReport({status:'CONNECTED', ...data, next:'Generate code blocks, then click Apply files to runner, then Run real tests.'});
    toast('Runner connected');
  }catch(err){
    setRunnerReport({status:'OFFLINE', error:err.message, howToStart:'In this project folder run: npm install && npm run runner. Keep it running, then click Check runner again.'});
    toast('Runner offline');
  }
}

async function runnerApplyBlocks(){
  if(!state.aiCodeBlocks?.length){ toast('No extracted code blocks to apply'); return; }
  const blocks=state.aiCodeBlocks.map((b,i)=>({path:inferRunnerPath(b,i), lang:b.lang, label:b.label, code:b.code}));
  try{
    const data=await runnerRequest('/api/apply',{blocks});
    setRunnerReport({status:'FILES_APPLIED', ...data, next:'Click Run real tests.'});
    toast(`Runner wrote ${data.written?.length || 0} file(s)`);
  }catch(err){
    setRunnerReport({status:'APPLY_FAILED', error:err.message});
    toast('Runner apply failed');
  }
}

async function runnerRunTests(){
  try{
    const data=await runnerRequest('/api/run-tests',{});
    setRunnerReport(data);
    if(data.status==='FAILED'){
      const prompt=`The local runner executed the generated project files and tests failed.\n\nRunner report:\n${JSON.stringify(data,null,2).slice(0,7000)}\n\nPlease return corrected files only, each with a File: path heading and fenced code block.`;
      document.getElementById('aiPromptInput').value=prompt;
      showTab('ai-build');
      toast('Tests failed — repair prompt loaded');
    } else {
      toast('Real tests passed');
    }
  }catch(err){
    setRunnerReport({status:'TEST_RUN_FAILED', error:err.message});
    toast('Could not run tests');
  }
}

function runnerGuide(){
  setRunnerReport(`How to use the local runner\n\n1. Unzip this package.\n2. Open a terminal inside the wirestack folder.\n3. Run: npm install\n4. Run: npm run runner\n5. Open index.html in your browser or deploy the static UI.\n6. Generate code in AI Build.\n7. Click Apply files to runner.\n8. Click Run real tests, or use Auto build / run / fix for the full loop.\n9. If tests pass, click Package app to create a zip or executable when supported.\n\nThe runner writes files into ./workspace. It can run JavaScript syntax checks, JSON checks, Python py_compile if installed, SQL basic checks, npm install/test if package.json exists, Python unittest discovery, and packaging for static zip/Python PyInstaller/Electron when tools are available.`);
}
...
const FULL_WIDTH_TABS = new Set(['wizard', 'help', 'ai-team']);
...
// ─── CHAT-FIRST COMMAND ROUTER ───────────────────────────────────────────────
function classifyChatIntent(text=''){
  const t=String(text).toLowerCase();
  const has=(...words)=>words.some(w=>t.includes(w));
  if(has('fix','repair','error','broken','conflict','conflicted','failing','failed')) return {type:'repair', label:'Repair problems', confidence:.9};
  if(has('test','tests','testing','spec','jest','edge case','coverage')) return {type:'test', label:'Write/run tests', confidence:.86};
  if(has('explain','what is','what does','how does','understand','where do i','what happened','what has it done')) return {type:'explain', label:'Explain / guide', confidence:.84};
  if(has('import','read files','old project','existing project','folder','repo','repository')) return {type:'import', label:'Import / understand project', confidence:.78};
  if(has('deploy','render','vercel','netlify','railway','publish','host')) return {type:'deploy', label:'Deployment help', confidence:.78};
  if(has('add','build','make','create','implement','change','update','improve','better','feature')) return {type:'build', label:'Build/change app', confidence:.88};
  if(!state.blueprint) return {type:'create', label:'Create blueprint', confidence:.7};
  return {type:'chat', label:'General project chat', confidence:.55};
}

function chatSubjectFromText(text=''){
  const t=String(text).toLowerCase();
  const known=state.blueprint ? Object.keys(state.blueprint.sections||{}) : [];
  const hit=known.find(id=>t.includes(id.toLowerCase()) || t.includes(id.split('.').pop().toLowerCase()));
  if(hit) return hit;
  if(t.includes('payment')||t.includes('stripe')||t.includes('subscription')) return 'svc.payments';
  if(t.includes('invoice')||t.includes('recurring')) return 'svc.invoices';
  if(t.includes('dashboard')||t.includes('ui')) return 'ui.dashboard';
  if(t.includes('auth')||t.includes('login')) return 'auth';
  if(t.includes('database')||t.includes('db')||t.includes('schema')) return 'db.invoices';
  if(t.includes('email')||t.includes('reminder')) return 'svc.email';
  if(t.includes('test')) return 'tests';
  return null;
}

function ensureBlueprintFromChat(text){
  if(state.blueprint) return false;
  const brief=document.getElementById('brief');
  const task=document.getElementById('task');
  if(brief) brief.value = String(text||brief.value||'Build a useful full-stack application.').trim();
  if(task) task.value = 'Create the first safe blueprint and build plan.';
  generateBlueprint();
  return true;
}

function moduleBlueprintForChat(id, text){
  const lower=String(text).toLowerCase();
  if(id==='svc.recurringInvoices') return {id,type:'service',layer:'Business logic',purpose:'Manage recurring invoice lifecycle, validation, creation, cancellation and sync.',dependsOn:['db.recurringInvoices','svc.payments'],usedBy:['api.recurringInvoices','tests'],code:'// Recurring invoice service will be generated by AI Build.'};
  if(id==='db.recurringInvoices') return {id,type:'database',layer:'Data',purpose:'Store recurring invoice templates, Stripe IDs, generated invoice links and audit events.',dependsOn:['db.invoices'],usedBy:['svc.recurringInvoices'],code:'-- Recurring invoice migration will be generated by AI Build.'};
  if(id==='api.recurringInvoices') return {id,type:'backend',layer:'API',purpose:'Expose create, list, cancel and webhook routes for recurring invoices.',dependsOn:['auth','svc.recurringInvoices'],usedBy:['ui.dashboard'],code:'// Recurring invoice API routes will be generated by AI Build.'};
  if(id==='ui.recurringInvoices') return {id,type:'frontend',layer:'Experience',purpose:'Dashboard panel for creating, listing and cancelling recurring invoices.',dependsOn:['api.recurringInvoices'],usedBy:['ui.dashboard'],code:'// Recurring invoice panel will be generated by AI Build.'};
  if(id==='tests.recurringInvoices') return {id,type:'quality',layer:'Safety',purpose:'Regression tests for recurring invoice creation, cancellation, webhook sync and edge cases.',dependsOn:['svc.recurringInvoices','api.recurringInvoices'],usedBy:['deploy'],code:'// Recurring invoice tests will be generated by AI Build.'};
  if(id==='svc.bookings') return {id,type:'service',layer:'Business logic',purpose:'Manage booking availability, creation, rescheduling and cancellation.',dependsOn:['db.bookings','svc.email'],usedBy:['api.bookings','tests'],code:'// Booking service will be generated by AI Build.'};
  if(id==='db.bookings') return {id,type:'database',layer:'Data',purpose:'Store appointments, customers, staff, services and booking status.',dependsOn:['db.users'],usedBy:['svc.bookings'],code:'-- Booking schema will be generated by AI Build.'};
  if(id==='api.bookings') return {id,type:'backend',layer:'API',purpose:'Expose booking endpoints for calendar and customer workflows.',dependsOn:['auth','svc.bookings'],usedBy:['ui.dashboard'],code:'// Booking API routes will be generated by AI Build.'};
  if(id==='svc.notifications') return {id,type:'service',layer:'External service',purpose:'Send reminders, confirmations and operational notifications.',dependsOn:['svc.email'],usedBy:['svc.bookings','svc.invoices'],code:'// Notification service will be generated by AI Build.'};
  return {id,type:'service',layer:'Business logic',purpose:`New module inferred from chat request: ${String(text).slice(0,160)}`,dependsOn:[],usedBy:['tests'],code:`// ${id} implementation will be generated by AI Build.`};
}

function addModuleIfMissing(id, text){
  const bp=state.blueprint; if(!bp) return false;
  if(bp.sections[id]) return false;
  const mod=moduleBlueprintForChat(id, text);
  bp.sections[id]=mod;
  bp.index.push({id:mod.id,order:bp.index.length+1,type:mod.type,layer:mod.layer});
  (mod.dependsOn||[]).forEach(dep=>{ if(bp.sections[dep] && !bp.wiring.some(w=>w[0]===id&&w[1]===dep)) bp.wiring.push([id,dep]); });
  return true;
}

function inferModulesForChatChange(text){
  const t=String(text).toLowerCase();
  const modules=[];
  if(t.includes('recurring') || t.includes('subscription') || t.includes('stripe')) modules.push('db.recurringInvoices','svc.recurringInvoices','api.recurringInvoices','ui.recurringInvoices','tests.recurringInvoices');
  if(t.includes('booking') || t.includes('calendar') || t.includes('appointment')) modules.push('db.bookings','svc.bookings','api.bookings');
  if(t.includes('reminder') || t.includes('notification')) modules.push('svc.notifications');
  if(t.includes('payment') && !modules.includes('svc.payments')) modules.push('svc.payments');
  if(t.includes('email') && !modules.includes('svc.email')) modules.push('svc.email');
  if(t.includes('dashboard') && !modules.includes('ui.dashboard')) modules.push('ui.dashboard');
  return [...new Set(modules)];
}

function applyChatBlueprintIntent(text){
  ensureBlueprintFromChat(text);
  const bp=state.blueprint; if(!bp) return {created:[],changed:[],wiring:[]};
  if(!state.blueprintBaseline) captureBlueprintBaseline(false);
  const created=[]; const changed=[]; const addedWires=[];
  inferModulesForChatChange(text).forEach(id=>{ if(addModuleIfMissing(id,text)) created.push(id); else changed.push(id); });
  // Keep common recurring invoice wiring explicit.
  const wires=[['svc.recurringInvoices','db.recurringInvoices'],['svc.recurringInvoices','svc.payments'],['api.recurringInvoices','svc.recurringInvoices'],['ui.recurringInvoices','api.recurringInvoices'],['tests.recurringInvoices','svc.recurringInvoices']];
  wires.forEach(([a,b])=>{ if(bp.sections[a]&&bp.sections[b]&&!bp.wiring.some(w=>w[0]===a&&w[1]===b)){ bp.wiring.push([a,b]); addedWires.push(`${a} -> ${b}`); } });
  bp.context.currentTask=String(text).slice(0,260);
  bp.changelog.push({date:today(),actor:'Chat Command Router',change:`Understood request and updated blueprint intent: ${String(text).slice(0,180)}`});
  state.layoutCache={key:null,pos:null};
  try{ runValidation(false); }catch{}
  try{ if(!state.testPlan) buildTestPlan(); runTestingEngine(false); }catch{}
  try{ runQutriScan(false); }catch{}
  try{ runBlueprintDiff(false); }catch{}
  render(); autosave();
  return {created,changed,wiring:addedWires};
}

function buildOutcomeReport(intent, text, local){
  const lines=[];
  lines.push(`WireStack understood this as: ${intent.label} (${Math.round(intent.confidence*100)}% confidence)`);
  if(local?.created?.length) lines.push(`\nBlueprint modules created:\n${local.created.map(x=>`- ${x}`).join('\n')}`);
  if(local?.changed?.length) lines.push(`\nExisting/affected modules:\n${local.changed.slice(0,10).map(x=>`- ${x}`).join('\n')}`);
  if(local?.wiring?.length) lines.push(`\nWiring added:\n${local.wiring.map(x=>`- ${x}`).join('\n')}`);
  if(state.validationReport) lines.push(`\nValidation: ${state.validationReport.status || 'checked'} · ${state.validationReport.summary?.errors||0} errors · ${state.validationReport.summary?.warnings||0} warnings`);
  if(state.testingReport) lines.push(`Testing confidence: ${state.testingReport.confidence ?? state.testingReport.overallConfidence ?? 'unknown'}`);
  if(state.qutri?.counts) lines.push(`Qutri: ${Object.entries(state.qutri.counts).map(([k,v])=>`${k}=${v}`).join(', ')}`);
  if(state.blueprintDiff) lines.push(`Blueprint diff: risk ${state.blueprintDiff.risk || 'unknown'} · health delta ${state.blueprintDiff.healthDelta ?? state.blueprintDiff.summary?.healthDelta ?? 'unknown'}`);
  if(getApiKey()) lines.push('\nNext: sending the grounded implementation request to the selected AI provider.');
  else lines.push('\nNext: add an API key, then press “Just send to AI” or “Do what I asked” to generate the actual code files for this plan.');
  return lines.join('\n');
}

function buildChatAiPrompt(intent, text, local){
  const subject=chatSubjectFromText(text);
  if(intent.type==='repair') return aiPromptTemplates.repairCurrent({module:subject,note:`User asked: ${text}\n\nDo not just explain. Provide corrected code blocks and state the files/sections to update.`});
  if(intent.type==='test') return aiPromptTemplates.writeTests({module:subject,note:`User asked: ${text}\n\nUse current blueprint/wiring and produce executable tests where possible.`});
  if(intent.type==='explain') return aiPromptTemplates.explainModule({module:subject,concept:subject||'current project',note:`User asked: ${text}\n\nExplain what has happened, whether code was generated, and what exact next step the user should take.`});
  if(intent.type==='deploy') return `Help deploy this WireStack/project state.\n\nUser asked: ${text}\n\nExplain exactly what files exist, what platform settings are needed, where API keys belong, and what is still missing before deployment.`;
  return `Do this request against the current WireStack blueprint.\n\nUser request:\n${text}\n\nLocal blueprint action already performed by WireStack:\n${JSON.stringify(local||{},null,2)}\n\nNow produce the actual implementation. Requirements:\n- Explain outcome first in plain English.\n- Return real code in fenced blocks, one block per file/section.\n- Include file paths above code blocks.\n- Include tests.\n- Update wiring/blueprint notes and mention risks.\n- End with exactly what the user should do next.`;
}

function updateIntentPreview(){
  const el=document.getElementById('intentPreview'); if(!el) return;
  const text=val('aiPromptInput'); const intent=classifyChatIntent(text); const subject=chatSubjectFromText(text);
  el.innerHTML=`<span class="intentChip">${escapeHtml(intent.label)}</span>${subject?` subject <b>${escapeHtml(subject)}</b> ·`:''} WireStack will route this through blueprint, wiring, validation, Qutri, tests and AI context.`;
}

function aiDoWhatAsked(){
  const text=val('aiPromptInput');
  setActiveAiRole(roleForIntentText(text));
  if(!text){ toast('Tell WireStack what you want first'); return; }
  const intent=classifyChatIntent(text);
  let local={created:[],changed:[],wiring:[]};
  if(['create','build','chat'].includes(intent.type) || /add|build|make|create|implement|change|update|improve/i.test(text)) local=applyChatBlueprintIntent(text);
  else if(intent.type==='repair'){ ensureBlueprintFromChat(text); try{runValidation(false)}catch{} try{runQutriScan(false)}catch{} try{runBlueprintDiff(false)}catch{} render(); }
  else if(intent.type==='test'){ ensureBlueprintFromChat(text); try{buildTestPlan(); runTestingEngine(false)}catch{} render(); }
  else if(intent.type==='explain'){ ensureBlueprintFromChat(text); }
  const report=buildOutcomeReport(intent,text,local);
  document.getElementById('aiOutput').textContent=report;
  state.aiConversation.push({role:'user',content:text});
  state.aiConversation.push({role:'assistant',content:report});
  document.getElementById('aiPromptInput').value=buildChatAiPrompt(intent,text,local);
  updateActiveContextHint(intent.type==='repair'?'repairCurrent':intent.type==='test'?'writeTests':intent.type==='explain'?'explainModule':'buildBlueprint',{module:chatSubjectFromText(text),note:'Prepared by chat command router'});
  renderAiHistory(); updateIntentPreview(); autosave();
  if(getApiKey()) setTimeout(()=>aiSend(),80);
}

function explainNextAction(){
  const lines=[];
  if(!state.blueprint) lines.push('Start by describing the app in Wizard or AI Build, then ask WireStack to create the blueprint.');
  else if(state.validationReport?.summary?.errors) lines.push('Current best next action: fix validation errors. Click “Do what I asked” with: Fix current errors.');
  else if(state.qutri?.counts?.conflicted) lines.push('Current best next action: repair conflicted Qutri modules before building further.');
  else if(!state.aiCodeBlocks?.length) lines.push('Current best next action: ask AI to generate real files from the blueprint.');
  else lines.push('Current best next action: review extracted code blocks, apply/export them, then run validation/testing again.');
  lines.push('\nTry one of these chat commands:\n- Build the first working app from this blueprint\n- Add recurring invoices with Stripe\n- Fix the current errors\n- Explain what changed\n- Write tests for invoices');
  document.getElementById('aiOutput').textContent=lines.join('\n');
  showTab('ai-build');
}



// ─── CHAT-FIRST AUTO BUILD / RUN / FIX PIPELINE ──────────────────────────────
function sleep(ms){ return new Promise(resolve=>setTimeout(resolve, ms)); }
function setAiOutputText(text){ const el=document.getElementById('aiOutput'); if(el) el.textContent=text; }
function appendAiOutputText(text){ const el=document.getElementById('aiOutput'); if(el) el.textContent += (el.textContent?'\n':'') + text; }
function summarizeRunnerFailure(data){
  const failed=(data?.failed||[]).slice(0,6).map(f=>({file:f.file,cmd:f.cmd,code:f.code,stderr:String(f.stderr||'').slice(0,1200),stdout:String(f.stdout||'').slice(0,600)}));
  return JSON.stringify({status:data?.status, failed, workspace:data?.workspace}, null, 2);
}
function buildAutoRepairPrompt(testData, originalRequest){
  return `WireStack ran the generated files in the local runner and tests/syntax checks failed.\n\nOriginal user request:\n${originalRequest}\n\nCompact runner failure report:\n${summarizeRunnerFailure(testData)}\n\nPlease repair the project. Return ONLY changed or new files. For every file, put a heading like \"File: path/to/file.ext\" immediately before a fenced code block. Include tests if needed. Do not return long explanations.`;
}
function waitForAiCompletion(prompt){
  setActiveAiRole(roleForIntentText(prompt));
  return new Promise((resolve,reject)=>{
    const oldBlocks=[...(state.aiCodeBlocks||[])];
    const outputEl=document.getElementById('aiOutput');
    document.getElementById('aiPromptInput').value=prompt;
    const thinkingEl=document.getElementById('aiThinking');
    const sendBtn=document.getElementById('aiSendBtn');
    ...
    sendToAiStreaming(
      prompt,
      text=>{ thinkingEl.style.display='none'; outputEl.textContent=text; outputEl.scrollTop=outputEl.scrollHeight; },
      fullText=>{
        sendBtn.disabled=false; stopBtn.style.display='none'; thinkingEl.style.display='none';
        if(!fullText){ state.aiCodeBlocks=oldBlocks; return reject(new Error('AI request stopped.')); }
        const blocks=extractAllFencedBlocks(fullText);
        state.aiCodeBlocks=blocks;
        renderAiCodeBlocks(blocks);
        if(blocks.length>0){
          applyBtn.style.display='inline-flex';
          document.getElementById('runnerApplyBtn').style.display='inline-flex';
          document.getElementById('runnerTestBtn').style.display='inline-flex';
          const pbtn=document.getElementById('runnerPackageBtn'); if(pbtn) pbtn.style.display='inline-flex';
        }
        renderAiHistory(); autosave(); resolve({fullText, blocks});
      },
      errMsg=>{
        sendBtn.disabled=false; stopBtn.style.display='none'; thinkingEl.style.display='none';
        outputEl.textContent='⚠ '+errMsg; reject(new Error(errMsg));
      }
    );
  });
}
async function runnerInstallDeps(){
  try{ const data=await runnerRequest('/api/install',{}); setRunnerReport(data); toast(data.status==='PASSED'?'Dependencies installed/checked':'Dependency install failed'); return data; }
  catch(err){ const data={status:'INSTALL_FAILED', error:err.message}; setRunnerReport(data); return data; }
}
async function runnerPackageApp(){
  try{ const data=await runnerRequest('/api/package',{}); state.lastPackage=data; setRunnerReport(data); toast(data.status==='PASSED'?'Package created':'Package step failed'); return data; }
  catch(err){ const data={status:'PACKAGE_FAILED', error:err.message}; state.lastPackage=data; setRunnerReport(data); toast('Package failed'); return data; }
}
async function runnerFullPipeline(){
  if(state.aiCodeBlocks?.length) await runnerApplyBlocks();
  const install=await runnerInstallDeps();
  const tests=await runnerRequest('/api/run-tests',{}).catch(err=>({status:'FAILED', failed:[{cmd:'runner', stderr:err.message}]}));
  setRunnerReport(tests);
  if(tests.status==='PASSED') return await runnerPackageApp();
  return tests;
}
async function autoBuildRunFixPackage(){
  const originalRequest=val('aiPromptInput');
  if(!originalRequest){ toast('Tell WireStack what to build first'); return; }
  if(!getApiKey()){ toast('Add an API key first'); setAiOutputText('Add your AI API key first, then press Auto build / run / fix again.'); return; }
  state.autoBuild.running=true; state.autoBuild.lastStatus='RUNNING';
  const log=[]; const say=(msg)=>{ log.push(msg); setAiOutputText(log.join('\n')); };
  try{
    showTab('ai-build');
    say('1. Understanding request and updating blueprint…');
    const intent=classifyChatIntent(originalRequest);
    const local=applyChatBlueprintIntent(originalRequest);
    say(buildOutcomeReport(intent, originalRequest, local));
    say('\n2. Asking AI to generate real project files…');
    const firstPrompt=`You are WireStack. Build a runnable app or change-set from this request.\n\nUser request:\n${originalRequest}\n\nCurrent blueprint/action:\n${JSON.stringify(local,null,2)}\n\nReturn real files only where possible. Use headings like \"File: package.json\" before fenced code blocks. Include a minimal package.json or requirements file, runnable code, and tests. If it is a static app, include index.html, style.css and app.js. If it is Node, include package.json and tests. Do not just explain.`;
    let result=await waitForAiCompletion(firstPrompt);
    say(`\n3. Extracted ${result.blocks.length} file/code block(s). Applying to local runner…`);
    if(!result.blocks.length) throw new Error('AI returned no code blocks to apply.');
    let apply=await runnerApplyBlocks();
    say('\n4. Installing/checking dependencies…');
    await runnerInstallDeps();
    let tests;
    for(let i=0;i<state.autoBuild.maxRepairLoops;i++){
      say(`\n5.${i+1} Running real tests/syntax checks…`);
      tests=await runnerRequest('/api/run-tests',{}).catch(err=>({status:'FAILED', failed:[{cmd:'runner', stderr:err.message}]}));
      setRunnerReport(tests);
      if(tests.status==='PASSED'){ say('\n✅ Tests passed.'); break; }
      say(`\n⚠ Tests failed. Repair loop ${i+1}/${state.autoBuild.maxRepairLoops}…`);
      const repairPrompt=buildAutoRepairPrompt(tests, originalRequest);
      result=await waitForAiCompletion(repairPrompt);
      if(!result.blocks.length) throw new Error('Repair AI returned no code blocks.');
      await runnerApplyBlocks();
      await runnerInstallDeps();
    }
    if(!tests || tests.status!=='PASSED'){
      state.autoBuild.lastStatus='FAILED';
      say('\n❌ Auto build stopped: tests still failing. The latest runner report has been loaded and a repair prompt is ready.');
      document.getElementById('aiPromptInput').value=buildAutoRepairPrompt(tests||{}, originalRequest);
      return;
    }
    say('\n6. Packaging because tests passed…');
    const pack=await runnerPackageApp();
    if(pack.status==='PASSED') say(`\n🎉 Finished. Package created in runner workspace.\n${JSON.stringify(pack.artifacts||pack,null,2)}`);
    else say(`\nTests passed but packaging did not complete.\n${JSON.stringify(pack,null,2)}`);
    try{ runValidation(false); runQutriScan(false); runBlueprintDiff(false); render(); }catch{}
    state.autoBuild.lastStatus='PASSED'; autosave();
  }catch(err){
    state.autoBuild.lastStatus='ERROR'; appendAiOutputText('\n\n⚠ Auto pipeline error: '+err.message); toast('Auto pipeline stopped');
  }finally{ state.autoBuild.running=false; }
}
...
document.getElementById('aiTeamMode')?.addEventListener('change', saveAiTeamFromUi);
document.getElementById('aiTeamSaveBtn')?.addEventListener('click', saveAiTeamFromUi);
document.getElementById('aiTeamRecommendedBtn')?.addEventListener('click', useRecommendedAiTeam);
document.getElementById('aiTeamApplyBuilderBtn')?.addEventListener('click',()=>{ saveAiTeamFromUi(); setActiveAiRole('builder'); showTab('ai-build'); toast('Builder role selected for chat'); });
...
document.getElementById('aiDoBtn')?.addEventListener('click', aiDoWhatAsked);
document.getElementById('aiAutoBuildBtn')?.addEventListener('click', autoBuildRunFixPackage);
document.getElementById('aiExplainNextBtn')?.addEventListener('click', explainNextAction);
document.getElementById('aiPromptInput')?.addEventListener('input', updateIntentPreview);
...
document.getElementById('runnerPingBtn')?.addEventListener('click', runnerPing);
document.getElementById('runnerApplyBtn')?.addEventListener('click', runnerApplyBlocks);
document.getElementById('runnerTestBtn')?.addEventListener('click', runnerRunTests);
document.getElementById('runnerPackageBtn')?.addEventListener('click', runnerPackageApp);
document.getElementById('runnerPackageDirectBtn')?.addEventListener('click', runnerPackageApp);
document.getElementById('runnerFullPipelineBtn')?.addEventListener('click', runnerFullPipeline);
document.getElementById('runnerOpenGuideBtn')?.addEventListener('click', runnerGuide);
