...
async function callOpenAI(model, messages, apiKeys = {}) {
  const key = apiKeys.openai || process.env.OPENAI_API_KEY;
  if (!key) throw new Error('OpenAI API key is not set. Add your key in the app under AI Keys.');
...
async function callAnthropic(model, messages, apiKeys = {}) {
  const key = apiKeys.anthropic || process.env.ANTHROPIC_API_KEY;
  if (!key) throw new Error('Claude/Anthropic API key is not set. Add your key in the app under AI Keys.');
...
async function runRole(role, userPrompt, apiKeys = {}) {
  ...
  if (cfg.provider === 'anthropic') return await callAnthropic(cfg.model, messages, apiKeys);
  return await callOpenAI(cfg.model, messages, apiKeys);
}
...
async function buildFullAppFromIdea(text, options = {}) {
  const stages = [];
  ...
  const result = await handleCommand(prompt, { autoApply:true, skipFullAppShortcut:true, apiKeys: options.apiKeys || {} });
...
async function handleCommand(text, options={}) {
  if (!options.skipFullAppShortcut && /full app|finished app|complete app|from idea.*works|build.*works/i.test(text)) return await buildFullAppFromIdea(text, options);
  const before = snapshotBlueprint();
  ...
  inferBlueprintFromText(text);
  try {
    aiText = await runRole(role, `${text}\n\nIntent: ${intent}. Work through it. If code is needed, produce complete files with File: headers.`, options.apiKeys || {});
  } catch (e) {
  ...
  if (written.length) {
    ...
    if (tests.status === 'FAILED' && ((options.apiKeys || {}).openai || process.env.OPENAI_API_KEY)) {
      try {
        repairText = await runRole('builder', `The generated files failed tests. Repair them.\n\nFailures:\n${JSON.stringify(tests.failed, null, 2).slice(0,12000)}`, options.apiKeys || {});
...
function publicState() { updateQutriAndHealth(); return { blueprint: state.blueprint, qutri: state.qutri, health: state.health, lastDiff: state.lastDiff, files: listFiles(), aiMode: state.aiMode, roles: state.roles, hasKeys: { openai: !!process.env.OPENAI_API_KEY, anthropic: !!process.env.ANTHROPIC_API_KEY }, keyMode: 'user-or-server' }; }
...
const server = http.createServer(async (req,res)=>{
  ...
  try {
    ...
    if (req.method === 'POST' && req.url === '/api/command') { const body = await readJson(req); return send(res,200,await handleCommand(body.text || '', { autoApply: body.autoApply !== false, apiKeys: body.apiKeys || {} })); }
