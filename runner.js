...
const PORT = Number(process.env.PORT || process.env.WIRESTACK_RUNNER_PORT || 8787);
...
function tryServeStatic(req, res) {
  ...
  try { urlPath = decodeURIComponent(urlPath); } catch (_) { return false; }
  const clean = urlPath.replace(/^\/+/, '');
  if (!clean || clean.includes('..')) return false;
  const abs = path.resolve(STATIC_ROOT, clean);
  if (!abs.startsWith(STATIC_ROOT + path.sep) && abs !== STATIC_ROOT) return false;
